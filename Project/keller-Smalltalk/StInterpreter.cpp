...
#define ST_TRACE_BYTECODE(msg)                                                 \
  qDebug() << "Bytecode"                                                       \
           << ("<" + QByteArray::number(currentBytecode) + ">").constData()    \
           << "\t" << __FUNCTION__ << "\t" << msg;
#define ST_TRACE_PRIMITIVE(msg)                                                \
  qDebug() << "Primitive"                                                      \
           << ("#" + QByteArray::number(primitiveIndex)).constData()           \
           << __FUNCTION__ << msg;
#define ST_TRACE_METHOD_CALL                                                   \
  qDebug()                                                                     \
      << level << ("[cycle=" + QByteArray::number(cycleNr) + "]").constData()  \
      << memory->prettyValue(stackValue(argumentCount)).constData()            \
      << QByteArray::number(memory->getRegister(NewMethod), 16).constData()    \
      << memory->fetchByteArray(memory->getRegister(MessageSelector))          \
             .constData()                                                      \
      << prettyArgs_().constData();
#define ST_RETURN_BYTECODE(msg)
#else
#define ST_TRACE_METHOD_CALL                                                   \
  qDebug()                                                                     \
      << level /*<< QByteArray(level,'\t').constData() */                      \
      ...
      << prettyArgs_().constData();
#define ST_RETURN_BYTECODE(msg)                                                \
  qDebug() << level /*<< QByteArray(level,'\t').constData() */ << "^"          \
           << ("<" + QByteArray::number(currentBytecode) + ">").constData()    \
           << msg;
#define ST_TRACE_BYTECODE(msg)
#ifdef ST_TRACE3_PRIMITIVES
#define ST_TRACE_PRIMITIVE(msg)                                                \
  qDebug() << level << "Primitive"                                             \
           << ("#" + QByteArray::number(primitiveIndex)).constData()           \
           << __FUNCTION__ << msg;
#else
#define ST_TRACE_PRIMITIVE(msg)
...
Interpreter::Interpreter(QObject *p)
    : QObject(p), memory(0), stackPointer(0), instructionPointer(0),
...
static ObjectMemory2::OOP findDisplay(ObjectMemory2 *memory) {
  ObjectMemory2::OOP sysdict =
      memory->fetchPointerOfObject(1, ObjectMemory2::smalltalk);
...
static Bitmap fetchBitmap(ObjectMemory2 *memory, Interpreter::OOP form) {
  ...
  return Bitmap(const_cast<quint8 *>(bs.d_bytes), bs.getWordLen(), width,
                height);
}
...
void Interpreter::interpret() {
  ...
  // apparently the system is in ScreenController
  // ->startUp->controlLoop->controlActivity->yellowButtonActivity
...
qint16
Interpreter::instructionPointerOfContext(Interpreter::OOP contextPointer) {
  return fetchIntegerOfObject(InstructionPointerIndex, contextPointer);
}

void Interpreter::storeInstructionPointerValueInContext(
    qint16 value, Interpreter::OOP contextPointer) {
...
void Interpreter::storeStackPointerValueInContext(
    qint16 value, Interpreter::OOP contextPointer) {
...
bool Interpreter::isBlockContext(Interpreter::OOP contextPointer) {
  const OOP methodOrArguments =
      memory->fetchPointerOfObject(MethodIndex, contextPointer);
...
void Interpreter::fetchContextRegisters() {
  ...
  memory->setRegister(Receiver,
                      memory->fetchPointerOfObject(ReceiverIndex, homeContext));
  memory->setRegister(Method,
                      memory->fetchPointerOfObject(MethodIndex, homeContext));
...
void Interpreter::storeContextRegisters() {
  OOP activeContext = memory->getRegister(ActiveContext);
  if (activeContext) // deviation from BB since activeContext is null on first
                     // call
  {
    storeInstructionPointerValueInContext(instructionPointer + 1,
                                          activeContext);
    storeStackPointerValueInContext(stackPointer - TempFrameStart + 1,
                                    activeContext);
  }
}
...
void Interpreter::push(Interpreter::OOP object) {
  ...
  stackPointer++;
  memory->storePointerOfObject(stackPointer, memory->getRegister(ActiveContext),
                               object);
}

Interpreter::OOP Interpreter::popStack() {
  OOP stackTop = memory->fetchPointerOfObject(
      stackPointer, memory->getRegister(ActiveContext));
  stackPointer--;
  return stackTop;
}

Interpreter::OOP Interpreter::stackTop() {
  return memory->fetchPointerOfObject(stackPointer,
                                      memory->getRegister(ActiveContext));
}

Interpreter::OOP Interpreter::stackValue(qint16 offset) {
  return memory->fetchPointerOfObject(stackPointer - offset,
                                      memory->getRegister(ActiveContext));
}

void Interpreter::pop(quint16 number) { stackPointer -= number; }

void Interpreter::unPop(quint16 number) { stackPointer += number; }
...
Interpreter::OOP Interpreter::sender() {
  return memory->fetchPointerOfObject(SenderIndex,
                                      memory->getRegister(HomeContext));
}

Interpreter::OOP Interpreter::caller() {
  return memory->fetchPointerOfObject(
      CallerIndex,
      memory->getRegister(ActiveContext)); // BB states SenderIndex instead
}

Interpreter::OOP Interpreter::temporary(qint16 offset) {
  return memory->fetchPointerOfObject(offset + TempFrameStart,
                                      memory->getRegister(HomeContext));
}
...
bool Interpreter::lookupMethodInDictionary(Interpreter::OOP dictionary) {
  ...
  while (true) {
    ...
    if (nextSelector == messageSelector) {
      const OOP methodArray =
          memory->fetchPointerOfObject(MethodArrayIndex, dictionary);
      OOP newMethod =
          memory->fetchPointerOfObject(index - SelectorStart, methodArray);
      memory->setRegister(NewMethod, newMethod);
      primitiveIndex = memory->primitiveIndexOf(newMethod);
      return true;
    }
...
bool Interpreter::lookupMethodInClass(Interpreter::OOP cls) {
  ...
  while (currentClass != ObjectMemory2::objectNil) {
    OOP dictionary =
        memory->fetchPointerOfObject(MessageDictionaryIndex, currentClass);
  ...
  if (memory->getRegister(MessageSelector) ==
      ObjectMemory2::symbolDoesNotUnderstand) {
  ...
  qCritical() << "ERROR: class" << memory->prettyValue(cls)
              << "doesNotUnderstand" << memory->prettyValue(selector);
  // exit(-1);
...
Interpreter::OOP
Interpreter::instanceSpecificationOf(Interpreter::OOP classPointer) {
...
quint8 Interpreter::fetchByte() {
  ...
  return memory->fetchByteOfObject(instructionPointer++,
                                   memory->getRegister(Method));
}
...
void Interpreter::onBreak() {
  ...
  r["success"] =
      success ? ObjectMemory2::objectTrue : ObjectMemory2::objectFalse;
  r["newProcessWaiting"] = newProcessWaiting ? ObjectMemory2::objectTrue
                                             : ObjectMemory2::objectFalse;
...
void Interpreter::checkProcessSwitch() {
  ...
  if (newProcessWaiting) {
    ...
    if (activeProcess_)
      memory->storePointerOfObject(SuspendedContextIndex, activeProcess_,
                                   memory->getRegister(ActiveContext));
    ...
    newActiveContext(
        memory->fetchPointerOfObject(SuspendedContextIndex, newProcess));
...
bool Interpreter::pushReceiverVariableBytecode() {
  ...
  push(memory->fetchPointerOfObject(extractBits(12, 15, currentBytecode),
                                    receiver));
...
bool Interpreter::pushTemporaryVariableBytecode() {
  ...
  ST_TRACE_BYTECODE(
      "variable:" << var << "value:" << memory->prettyValue(val).constData());
...
bool Interpreter::pushLiteralConstantBytecode() {
  ...
  ST_TRACE_BYTECODE(
      "literal:"
      << fieldIndex << "value:"
      << memory->prettyValue(literalConstant).constData() << "of method:"
      << QByteArray::number(memory->getRegister(Method), 16).constData());
...
bool Interpreter::pushLiteralVariableBytecode() {
  ...
  ST_TRACE_BYTECODE(
      "literal:"
      << fieldIndex << "value:" << memory->prettyValue(value).constData()
      << "of method:"
      << QByteArray::number(memory->getRegister(Method), 16).constData());
  push(value);
  return true;
...
bool Interpreter::storeAndPopReceiverVariableBytecode() {
  ...
  OOP val = popStack();
  ST_TRACE_BYTECODE("var:" << variableIndex
                           << "val:" << memory->prettyValue(val).constData());
  memory->storePointerOfObject(variableIndex, memory->getRegister(Receiver),
                               val);
  return true;
}
...
bool Interpreter::storeAndPopTemporaryVariableBytecode() {
  ...
  ST_TRACE_BYTECODE("var:" << variableIndex
                           << "val:" << memory->prettyValue(val).constData());
  memory->storePointerOfObject(variableIndex + TempFrameStart,
                               memory->getRegister(HomeContext), val);
  return true;
}
...
static const char *s_constNames[] = {"???", "true", "false", "nil",
                                     "-1",  "0",    "1",     "2"};
...
bool Interpreter::extendedPushBytecode() {
  // "Push (Receiver Variable, Temporary Location, Literal Constant, Literal
  // Variable) [%1] #%2"). arg( ( bc[pc+1] >> 6 ) & 0x3 ).arg( bc[pc+1] & 0x3f),
  // 2 );
  ...
  switch (variableType) {
  case 0:
    val = memory->fetchPointerOfObject(variableIndex,
                                       memory->getRegister(Receiver));
    break;
  case 1:
...
bool Interpreter::extendedStoreBytecode(bool subcall) {
  ...
  // "Store (Receiver Variable, Temporary Location, Illegal, Literal Variable)
  // [%1] #%2"). arg( ( bc[pc+1] >> 6 ) & 0x3 ).arg( bc[pc+1] & 0x3f), 2 );
  const quint16 descriptor = fetchByte();
  const quint16 variableType = extractBits(8, 9, descriptor);
  const quint16 variableIndex = extractBits(10, 15, descriptor);
  switch (variableType) {
  ...
  case 1:
    memory->storePointerOfObject(variableIndex + TempFrameStart,
                                 memory->getRegister(HomeContext), stackTop());
    break;
  case 2:
    ...
    break;
  case 3:
    memory->storePointerOfObject(ValueIndex, literal(variableIndex),
                                 stackTop());
    break;
  default:
    Q_ASSERT(false);
...
bool Interpreter::extendedStoreAndPopBytecode() {
  ST_TRACE_BYTECODE("");
  // "Pop and Store (Receiver Variable, Temporary Location, Illegal, Literal
  // Variable) [%1] #%2"). arg( ( bc[pc+1] >> 6 ) & 0x3 ).arg( bc[pc+1] & 0x3f),
  // 2 );
...
bool Interpreter::singleExtendedSendBytecode() {
  // "Send Literal Selector #%2 With %1 Arguments").arg( ( bc[pc+1] >> 5 ) & 0x7
  // ).arg( bc[pc+1] & 0x1f), 2 );
  const quint16 descriptor = fetchByte();
...
bool Interpreter::doubleExtendedSendBytecode() {
  // "Send Literal Selector #%2 With %1 Arguments").arg( bc[pc+1] ).arg(
  // bc[pc+2]), 3 );
...
bool Interpreter::singleExtendedSuperBytecode() {
  // "Send Literal Selector #%2 To Superclass With %1 Arguments").arg( (
  // bc[pc+1] >> 5 ) & 0x7 ).arg( bc[pc+1] & 0x1f), 2 );
  const quint16 descriptor = fetchByte();
  ...
  ST_TRACE_BYTECODE("selector:" << memory->prettyValue(selector).constData()
                                << "super:"
                                << memory->prettyValue(super).constData());
...
bool Interpreter::doubleExtendedSuperBytecode() {
  ST_TRACE_BYTECODE("");
  // "Send Literal Selector #%2 To Superclass With %1 Arguments").arg( bc[pc+1]
  // ).arg( bc[pc+2]), 3 );
...
bool Interpreter::sendSpecialSelectorBytecode() {
  ...
  if (!specialSelectorPrimitiveResponse()) {
    ...
    OOP selector = memory->fetchPointerOfObject(
        selectorIndex, ObjectMemory2::specialSelectors);
    const quint16 count = fetchIntegerOfObject(selectorIndex + 1,
                                               ObjectMemory2::specialSelectors);
    ST_TRACE_BYTECODE("selector:" << memory->prettyValue(selector).constData()
                                  << "count:" << count);
    sendSelector(selector, count);
  } else
...
void Interpreter::jumpif(quint16 condition, qint32 offset) {
  ...
  if (boolean == condition)
    jump(offset);
  else if (!(boolean == ObjectMemory2::objectTrue ||
             boolean == ObjectMemory2::objectFalse)) {
    unPop(1);
    sendMustBeBoolean();
  }
}
...
void Interpreter::sendSelectorToClass(Interpreter::OOP classPointer) {
  // deviation from BB, we currently don't have a methodCache, original:
...
void Interpreter::activateNewMethod() {
  ...
  if (memory->largeContextFlagOf(newMethod))
    ...
    contextSize += 12;
  OOP newContext = memory->instantiateClassWithPointers(
      ObjectMemory2::classMethodContext, contextSize);
  // qDebug() << "new MethodContext for method" <<
  // QByteArray::number(newMethod,16).constData() << "level" << level; // TEST
  OOP activeContext = memory->getRegister(ActiveContext);
  memory->storePointerOfObject(SenderIndex, newContext, activeContext);
  storeInstructionPointerValueInContext(
      memory->initialInstructionPointerOfMethod(newMethod), newContext);
  storeStackPointerValueInContext(memory->temporaryCountOf(newMethod),
                                  newContext);
  memory->storePointerOfObject(MethodIndex, newContext, newMethod);
  transfer(argumentCount + 1, stackPointer - argumentCount, activeContext,
           ReceiverIndex, newContext);
  pop(argumentCount + 1);
  newActiveContext(newContext);
}

void Interpreter::transfer(quint32 count, quint16 firstFrom,
                           Interpreter::OOP fromOop, quint16 firstTo,
                           Interpreter::OOP toOop) {
...
void Interpreter::nilContextFields() {
  memory->storePointerOfObject(SenderIndex, memory->getRegister(ActiveContext),
                               ObjectMemory2::objectNil);
  memory->storePointerOfObject(InstructionPointerIndex,
                               memory->getRegister(ActiveContext),
                               ObjectMemory2::objectNil);
}
...
void Interpreter::returnValue(Interpreter::OOP resultPointer,
  ...
  ST_TRACE_BYTECODE(
      "result:" << memory->prettyValue(resultPointer).constData() << "context:"
                << memory->prettyValue(contextPointer).constData());
  ...
  OOP sendersIP =
      memory->fetchPointerOfObject(InstructionPointerIndex, contextPointer);
...
void Interpreter::initPrimitive() { success = true; }

void Interpreter::successUpdate(bool res) { success &= res; }
...
Interpreter::OOP Interpreter::positive16BitIntegerFor(quint16 integerValue) {
  ...
  OOP newLargeInteger = memory->instantiateClassWithBytes(
      ObjectMemory2::classLargePositiveInteger, 2);
...
quint16 Interpreter::positive16BitValueOf(OOP integerPointer) {
  ...
  if (memory->fetchClassOf(integerPointer) !=
      ObjectMemory2::classLargePositiveInteger)
...
void Interpreter::commonSelectorPrimitive() {
  argumentCount = fetchIntegerOfObject((currentBytecode - 176) * 2 + 1,
                                       ObjectMemory2::specialSelectors);
...
static int
MOD(int a,
    int b) // Source: http://lists.inf.ethz.ch/pipermail/oberon/2019/013353.html
{
  Q_ASSERT(b > 0);
  if (a < 0)
    return (b - 1) + ((a - b + 1)) % b;
...
void Interpreter::primitiveMakePoint() {
  ...
  successUpdate(memory->isIntegerObject(
      integerArgument)); // BB error; apparently arg is an OOP, see also
                         // primitiveMousePoint
  successUpdate(memory->isIntegerObject(integerReceiver));

  if (success) {
    OOP pointResult = memory->instantiateClassWithPointers(
        ObjectMemory2::classPoint, ClassPointSize);
    ...
    // qDebug() << memory->prettyValue(integerArgument) <<
    // memory->prettyValue(integerReceiver); everytime we arrived here so far
    // integerArgument and integerReceiver were Float
    unPop(2);
  }
}
...
qint16 Interpreter::fetchIntegerOfObject(quint16 fieldIndex,
                                         Interpreter::OOP objectPointer) {
...
void Interpreter::storeIntegerOfObjectWithValue(quint16 fieldIndex,
                                                Interpreter::OOP objectPointer,
                                                int integerValue) {
...
void Interpreter::primitiveBlockCopy() {
  ...
  OOP newContext = memory->instantiateClassWithPointers(
      ObjectMemory2::classBlockContext, contextSize);
  ...
  memory->storePointerOfObject(BlockArgumentCountIndex, newContext,
                               blockArgumentCount);
...
void Interpreter::primitiveValue() {
  ...
  if (success) {
    ...
    memory->storePointerOfObject(InstructionPointerIndex, blockContext,
                                 initialIP);
    storeStackPointerValueInContext(argumentCount, blockContext);
    memory->storePointerOfObject(CallerIndex, blockContext,
                                 memory->getRegister(ActiveContext));
...
void Interpreter::quickInstanceLoad() {
  ...
  // qDebug() << "quickInstanceLoad index" << fieldIndex << "value" <<
  // memory->prettyValue(val).constData();
  push(val);
}
...
void Interpreter::dispatchPrivatePrimitives() {
  ...
  case 128:
    ...
    break;
  default:
    qWarning() << "WARNING: private primitive" << primitiveIndex
               << "not yet implemented";
    primitiveFail();
    break;
  }
...
void Interpreter::dispatchLargeIntegerPrimitives() { primitiveFail(); }
...
void Interpreter::_floatCompImp(char op) {
  ...
  if (success) {
    switch (op) {
    case '=':
      push(floatReceiver == floatArgument ? ObjectMemory2::objectTrue
                                          : ObjectMemory2::objectFalse);
      break;
    case '!':
      push(floatReceiver != floatArgument ? ObjectMemory2::objectTrue
...
void Interpreter::primitiveAt() {
  ...
  //    qDebug() << "primitiveAt receiver" <<
  //    memory->prettyValue(array).constData()
...
void Interpreter::primitiveAtPut() {
  ...
  //    qDebug() << "primitiveAtPut receiver" <<
  //    memory->prettyValue(array).constData()
  //             << "index" << memory->prettyValue(index).constData()
...
void Interpreter::primitiveStringAt() {
  ...
  if (success) {
    ...
    character =
        memory->fetchPointerOfObject(ascii, ObjectMemory2::characterTable);
  }
  if (success)
    push(character);
  else
...
void Interpreter::primitiveStringAtPut() {
  ...
  OOP array = popStack();
  checkIndexableBoundsOf(index, array);
  successUpdate(memory->fetchClassOf(character) ==
                ObjectMemory2::classCharacter);
  if (success) {
...
void Interpreter::primitiveNext() {
  ...
  successUpdate(arrayClass == ObjectMemory2::classArray ||
                arrayClass == ObjectMemory2::classString);
...
void Interpreter::primitiveNextPut() {
  ...
  if (success) {
    ...
    if (arrayClass == ObjectMemory2::classArray)
      ...
      OOP ascii =
          memory->fetchPointerOfObject(0, value); // character value index 0
      subscriptWithStoring(array, index, ascii);
    }
  }
  if (success)
...
void Interpreter::subscriptWithStoring(Interpreter::OOP array, int index,
                                       Interpreter::OOP value) {
  OOP cls = memory->fetchClassOf(array);
  if (isWords(cls)) {
    if (isPointers(cls))
      ...
      if (success)
        memory->storeWordOfObject(index - 1, array,
                                  positive16BitValueOf(value));
      // else never observed so far
    }
  } else {
    successUpdate(memory->isIntegerObject(value));
    if (success)
      memory->storeByteOfObject(index - 1, array,
                                lowByteOf(memory->integerValueOf(value)));
  }
}
...
void Interpreter::primitiveObjectAtPut() {
  ...
  //    qDebug() << "primitiveObjectAtPut receiver" <<
  //    memory->prettyValue(thisReceiver).constData()
  //             << "index" << memory->prettyValue(index).constData()
...
void Interpreter::primitiveNew() {
  ...
  if (success) {
    if (isPointers(cls))
      ...
      // qWarning() << "primitiveNew word" <<
      // memory->fetchClassName(cls).constData() << size; never called
...
void Interpreter::primitiveNewWithArg() {
  ...
  if (success) {
    ...
    if (isPointers(cls))
      ...
      // qWarning() << "primitiveNewWithArg word" <<
      // memory->fetchClassName(cls).constData() << size; WordArray,
      // DisplayBitmap
      push(memory->instantiateClassWithWords(cls, size));
    } else {
      // qWarning() << "primitiveNewWithArg bytes" <<
      // memory->fetchClassName(cls).constData() << size; LargePositiveInteger
      // (0,1,2 or 3 bytes observed), String
...
void Interpreter::primitiveBecome() {
  ...
  // qDebug() << "primitiveBecome" << memory->prettyValue(thisReceiver) <<
  // memory->prettyValue(otherPointer);
  if (success) {
...
void Interpreter::primitiveInstVarAtPut() {
  ...
  //    qDebug() << "primitiveInstVarAtPut receiver" <<
  //    memory->prettyValue(thisReceiver).constData()
  //             << "index" << memory->prettyValue(index).constData()
  //             << "value" << memory->prettyValue(newValue).constData();
...
void Interpreter::primitiveNewMethod() {
  ...
  memory->storeWordOfObject(0, newMethod,
                            header); // BB error: this line got obviously lost
  for (int i = 0; i < literalCount; i++)
    memory->storePointerOfObject(
        1 + i, newMethod, ObjectMemory2::objectNil); // BB error, VIM fixed
  push(newMethod);
}

void Interpreter::checkInstanceVariableBoundsOf(int index,
                                                Interpreter::OOP object) {
  // OOP cls = memory->fetchClassOf(object); // BB makes this fetch, but not
  // used
  successUpdate(index >= 1);
...
void Interpreter::primitiveValueWithArgs() {
  ...
  if (success) {
    transfer(arrayArgumentCount, 0, argumentArray, TempFrameStart,
             blockContext);
    OOP initialIP = memory->fetchPointerOfObject(InitialIPIndex, blockContext);
    memory->storePointerOfObject(InstructionPointerIndex, blockContext,
                                 initialIP);
...
void Interpreter::primitivePerform() {
  ...
  ST_TRACE_PRIMITIVE("selector"
                     << memory->prettyValue(newSelector).constData());
  OOP newReceiver = stackValue(argumentCount);
  lookupMethodInClass(memory->fetchClassOf(newReceiver));
  successUpdate(memory->argumentCountOf(memory->getRegister(NewMethod)) ==
                argumentCount - 1);
  if (success) {
    int selectorIndex = stackPointer - argumentCount + 1;
    transfer(argumentCount - 1, selectorIndex + 1,
             memory->getRegister(ActiveContext), selectorIndex,
             memory->getRegister(ActiveContext));
    pop(1);
    argumentCount--;
    executeNewMethod();
...
void Interpreter::primitivePerformWithArgs() {
  ...
  OOP arrayClass = memory->fetchClassOf(argumentArray);
  successUpdate((stackPointer + arraySize) <
                memory->fetchWordLenghtOf(memory->getRegister(ActiveContext)));
  successUpdate(arrayClass == ObjectMemory2::classArray);
  if (success) {
    OOP performSelector = memory->getRegister(MessageSelector);
...
void Interpreter::primitiveWait() {
  ...
  if (excessSignals > 0)
    storeIntegerOfObjectWithValue(ExcessSignalIndex, thisReceiver,
                                  excessSignals - 1);
  else {
...
bool Interpreter::isEmptyList(Interpreter::OOP aLinkedList) {
  if (aLinkedList == ObjectMemory2::objectNil)
    return true;
  return memory->fetchPointerOfObject(FirstLinkIndex, aLinkedList) ==
         ObjectMemory2::objectNil;
}
...
void Interpreter::synchronousSignal(Interpreter::OOP aSemaphore) {
  if (isEmptyList(aSemaphore)) {
    ...
    storeIntegerOfObjectWithValue(ExcessSignalIndex, aSemaphore,
                                  excessSignals + 1);
  } else
...
Interpreter::OOP
Interpreter::removeFirstLinkOfList(Interpreter::OOP aLinkedList) {
  ...
  if (firstLink == lastLink) {
    memory->storePointerOfObject(FirstLinkIndex, aLinkedList,
                                 ObjectMemory2::objectNil);
    memory->storePointerOfObject(LastLinkIndex, aLinkedList,
                                 ObjectMemory2::objectNil);
  } else {
  ...
  memory->storePointerOfObject(NextLinkIndex, firstLink,
                               ObjectMemory2::objectNil);
  return firstLink;
}
...
void Interpreter::addLastLinkToList(Interpreter::OOP aLink,
                                    Interpreter::OOP aLinkedList) {
...
Interpreter::OOP Interpreter::wakeHighestPriority() {
  OOP processLists =
      memory->fetchPointerOfObject(ProcessListIndex, schedulerPointer());
...
void Interpreter::suspendActive() { transferTo(wakeHighestPriority()); }
...
void Interpreter::createActualMessage() {
  OOP argumentArray = memory->instantiateClassWithPointers(
      ObjectMemory2::classArray, argumentCount);
  OOP message = memory->instantiateClassWithPointers(
      ObjectMemory2::classMessage, MessageSize);
  memory->storePointerOfObject(MessageSelectorIndex, message,
                               memory->getRegister(MessageSelector));
  memory->storePointerOfObject(MessageArgumentsIndex, message, argumentArray);
  transfer(argumentCount, stackPointer - (argumentCount - 1),
           memory->getRegister(ActiveContext), 0, argumentArray);
  pop(argumentCount);
  push(message);
  argumentCount = 1;
}
...
void Interpreter::primitiveCopyBits() {
  ...
  /* Stack:
  9 "<a BitBlt>" // there is always a BitBlt or one of its subclasses top of the
  stack 8 "<a Form>" 7 "3" 6 "<a Rectangle>"
  */
  OOP bitblt = stackTop();

  Bitmap destBits =
      fetchBitmap(memory, memory->fetchPointerOfObject(0, bitblt));
  Bitmap sourceBits =
      fetchBitmap(memory, memory->fetchPointerOfObject(1, bitblt));
  Bitmap halftoneBits =
      fetchBitmap(memory, memory->fetchPointerOfObject(2, bitblt));
  ...
  in.combinationRule =
      memory->integerValueOf(memory->fetchPointerOfObject(3, bitblt), true);
  in.destX =
      memory->integerValueOf(memory->fetchPointerOfObject(4, bitblt), true);
  in.destY =
      memory->integerValueOf(memory->fetchPointerOfObject(5, bitblt), true);
  in.width =
      memory->integerValueOf(memory->fetchPointerOfObject(6, bitblt), true);
  in.height =
      memory->integerValueOf(memory->fetchPointerOfObject(7, bitblt), true);
  in.sourceX =
      memory->integerValueOf(memory->fetchPointerOfObject(8, bitblt), true);
  in.sourceY =
      memory->integerValueOf(memory->fetchPointerOfObject(9, bitblt), true);
  in.clipX =
      memory->integerValueOf(memory->fetchPointerOfObject(10, bitblt), true);
  in.clipY =
      memory->integerValueOf(memory->fetchPointerOfObject(11, bitblt), true);
  in.clipWidth =
      memory->integerValueOf(memory->fetchPointerOfObject(12, bitblt), true);
  in.clipHeight =
      memory->integerValueOf(memory->fetchPointerOfObject(13, bitblt), true);

  //    if( !halftoneBits.isNull() )
  //        Q_ASSERT( halftoneBits.width() == 16 && halftoneBits.height() == 16
  //        ); // this always holds
  ...
  //    static int count = 0;
  //    destBits.toImage().save(QString("cppscreens/bitblt_%1.png").arg(++count,4,
  //    10, QChar('0')));

#ifdef ST_DO_SCREEN_RECORDING
  if (disp->isRecOn() && drawToDisp) {
    //        qDebug() << cycleNr << "updating screen" << in.destX << in.destY
    //        << in.width << in.height
    //                 << "clipped at" << in.clipX << in.clipY << in.clipWidth
    //                 << in.clipHeight;
    qDebug() << "copyBits" << in.destX << in.destY << in.width << in.height
             << in.combinationRule;
...
void Interpreter::dumpStack_(const char *title) {
  qWarning() << "**********" << title << "argcount" << argumentCount
             << "begin stack:";
#if 0
    for( int i = 0; i < argumentCount; i++ )
  ...
  for (int i = stackPointer; i > TempFrameStart; i--)
    qWarning() << i
               << memory
                      ->prettyValue(memory->fetchPointerOfObject(
                          i, memory->getRegister(ActiveContext)))
                      .constData();
#endif
  qWarning() << "***** end stack";
}
...
void Interpreter::primitiveCursorLink() {
  ST_TRACE_PRIMITIVE("");
  qWarning() << "WARNING: primitiveCursorLink not supported"
             << memory->prettyValue(stackTop());
  popStack(); // bool
}
...
void Interpreter::primitiveSignalAtOopsLeftWordsLeft() {
  ...
  qWarning() << "WARNING: primitiveSignalAtOopsLeftWordsLeft not implemented"
             << memory->prettyValue(number1).constData()
             << memory->prettyValue(number2).constData()
             << memory->prettyValue(semaphore).constData();
}

void Interpreter::primitiveCursorLocPut() {
  ST_TRACE_PRIMITIVE("");
  OOP point = popStack();
  Display::inst()->setCursorPos(
      memory->integerValueOf(memory->fetchPointerOfObject(XIndex, point)),
...
void Interpreter::primitiveTimeWordsInto() {
  ST_TRACE_PRIMITIVE("");
  OOP oop = popStack();
  const quint32 diff = QDateTime(QDate(1901, 1, 1), QTime(0, 0, 0))
...
void Interpreter::primitiveTickWordsInto() {
  ...
  // NOTE: unexpected byte order, but empirically validated with
  // primitiveSignalAtTick that it's the right one
...
void Interpreter::primitiveAltoFile() {
  ...
  12 4096             address: diskAddress // see
  AltoFileDirectory>>virtualToReal: 1->4096 11 0                diskNumber 10 <a
  AltoFile> 9 <a Semaphore> 8 "read:" 7 <a AltoFilePage>
  ***** end stack */
  ...
  ST_TRACE_PRIMITIVE("command:" << command << "address:" << address
                                << "disk:" << diskNumber << "bufsize:"
                                << memory->fetchByteLenghtOf(buffer));
...
void Interpreter::_compareImp(char op) {
  ...
  if (success) {
    switch (op) {
    case '=':
      push(integerReceiver == integerArgument ? ObjectMemory2::objectTrue
                                              : ObjectMemory2::objectFalse);
      break;
    case '!':
      push(integerReceiver != integerArgument ? ObjectMemory2::objectTrue
