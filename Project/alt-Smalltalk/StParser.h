...
protected:
  ...
  struct TokStream {
    ...
    Toks d_toks;
    quint32 d_pos;
    TokStream(const Toks &t = Toks(), quint32 pos = 0)
        : d_toks(t), d_pos(pos) {}
    Lexer::Token next();
  ...
  Ast::Ref<Ast::Expression> parseExpression(Ast::Function *, TokStream &,
                                            bool dontApplyKeywords = false);
...
private:
  Lexer *d_lex;
  QByteArray subclass, comment, methodsFor, instanceVariableNames,
      classVariableNames, poolDictionaries, category, class_, variableSubclass,
      variableByteSubclass, variableWordSubclass, primitive;
  QList<Error> d_errs;
