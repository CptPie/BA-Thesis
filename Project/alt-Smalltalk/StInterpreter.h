...
// later code reviewed based on this version:
...
public:
  ...
  enum MethodContext {
    SenderIndex =
        0, // BB: The suspended context is called the new context's sender
  ...
  enum ProcessScheduler { ProcessListIndex = 0, ActiveProcessIndex = 1 };
  ...
  enum LinkedList { FirstLinkIndex = 0, LastLinkIndex = 1 };

  enum Semaphore { ExcessSignalIndex = 2 };

  enum Link { NextLinkIndex = 0 };
...
protected:
  ...
  void transfer(quint32 count, quint16 fromIndex, OOP fromOop, quint16 firstTo,
                OOP toOop);
  ...
  void storeIntegerOfObjectWithValue(quint16 fieldIndex, OOP objectPointer,
                                     int integerValue);
